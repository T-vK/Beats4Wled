...
export default class WledRealtimeClient {
    ...
    update() {
        ...
        for (const packetIndices of idxSplit) {
            const data = Buffer.alloc(header.length + packetIndices.length * 5); // 2 bytes for index + 3 bytes for RGB
            header.copy(data, 0);
            let dataIndex = header.length;
            for (const i of packetIndices) {
                data.writeUInt16BE(i, dataIndex); // Write 16-bit index
                dataIndex += 2;
                p.copy(data, dataIndex, i * 3, i * 3 + 3); // Copy RGB data
                dataIndex += 3;
            }
